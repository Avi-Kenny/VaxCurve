...
cox_var <- function(dat, theta_n, z_0, t, tau=NA, return_extra=NA) {
  
  # theta_n=as.numeric(coeffs);z_0=c(0.5,1,0.5);t=100;
  ...
  infl_fn_marg <- function(w_i,delta_i,t_i,a) {
    
    piece_1 <- S_n(w_i[1],w_i[2],a)
    piece_2 <- (1/N) * sum(sapply(c(1:N), function(j) {
      S_n(Z_[1,j],Z_[2,j],a)
    }))
    piece_3 <- (1/N) * sum(sapply(c(1:N), function(j) {
      S_n(Z_[1,j],Z_[2,j],a) * 999
    }))
  ...
  return(list(
    ...
    var_cmhz_est = var_cmhz_est, # !!!!! var_cmhz_est or var_cmhz_est2 ?????
    var_bshz_est = var_bshz_est,
    var_surv_est = var_surv_est
  ))
